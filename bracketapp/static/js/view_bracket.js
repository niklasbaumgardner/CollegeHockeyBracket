...
class Matchup {
  constructor(game_num, top, bottom) {
    ...
    if (BRACKET.games[this.game_num]) {
      for (let [k, v] of Object.entries(BRACKET.games[this.game_num])) {
        this.game[k] = v;
      }
    }
  }
  ...
  render(anchor) {
    ...
    // create the top winner pick
    if (!this.isDefaultGame) {
      let additionalClassString = "";
      if (this.top.correct.winner) {
        additionalClassString +=
          this.top.correct.winner === this.top.game.winner
            ? " correct"
            : " incorrect";
      }
      this.topWinner = createElement({
        type: "li",
        classString: "team bg-body-secondary-nb" + additionalClassString,
    ...
    // create the bottom winner pick
    if (!this.isDefaultGame) {
      let additionalClassString = "";
      if (this.bottom.correct.winner) {
        additionalClassString +=
          this.bottom.correct.winner === this.bottom.game.winner
            ? " correct"
            : " incorrect";
      }
...
class FinalMatch extends Matchup {
  constructor(game_num, top, bottom) {
    ...
    for (let [k, v] of Object.entries(BRACKET)) {
      this[k] = v;
    }
  ...
  render(anchor) {
    ...
    this.winnerText = createElement({
      type: "p",
      classString: "fs-5",
      content: IS_ME ? "Your pick" : "Their pick",
    });
    this.colInnerRow.appendChild(this.winnerText);

    if (this.correct.winner) {
      if (this.correct.winner !== this.winner) {
        this.winnerImg.classList.add("greyscale");

        this.winnerCol = createElement({ classString: "col center-text" });
        this.topRow.appendChild(this.winnerCol);

        this.winnerInnerRow = createElement({
          classString: "row justify-content-center",
        });
        this.winnerCol.appendChild(this.winnerInnerRow);

        this.winnerInnerRow.appendChild(
          createElement({
            type: "img",
            classString: "winner-img",
            src: `/static/images/${CORRECT_WINNER_IMAGE}.svg`,
            alt: CORRECT_WINNER_IMAGE,
          })
        );
        this.winnerInnerRow.appendChild(
          createElement({
        ...
        this.winnerText.textContent += " & Champion";
      }
    }
