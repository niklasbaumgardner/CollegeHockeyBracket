import{d as t,h as C,j as ye,k as x}from"./chunk-XGX4NMMB.mjs";import{a as _e,b as $e,c as Ee,d as u}from"./chunk-NB5LNMQ2.mjs";import{a as d,b as Ce,d as xe,e as Se,f as Be}from"./chunk-SADXGP7D.mjs";function Ie(i,e){return()=>i.querySelector(e)}function Oe(i,e){return()=>i.querySelectorAll(e)}var n=class extends ye{constructor(){super();let{queries:e}=this.constructor;if(e)for(let[a,r]of Object.entries(e))r.all?Object.defineProperty(this,a,{get:Oe(this,r.all)}):Object.defineProperty(this,a,{get:Ie(this,r)})}createRenderRoot(){return this}};var S=class extends n{render(){return t`<wa-card
      ><form method="POST">
        <div class="wa-stack">
          <wa-input
            name="team"
    ...
    </wa-card>`}};customElements.define("nb-admin",B);var R=class extends n{static properties={archivedYears:{type:Array}};imgTemplate(e){return e?t`<img
      class="w-[75px] h-[75px]"
    ...
    >`}};customElements.define("nb-back-button",N);var G=class extends n{static properties={message:{type:String},category:{type:String}};connectedCallback(){super.connectedCallback(),this.removeSelfTask=new d(()=>this.removeSelf(),5e3,{finalizeBeforeUnload:!0}),this.removeSelfTask.arm()}removeSelf(){this.remove()}render(){return t`<wa-callout variant=${this.category}>
      ...
      >
    </wa-dropdown>`}};customElements.define("nb-theme-selector",O);var y=new Date("2025-03-27T18:00:00.000Z"),P=class extends n{static properties={bracketsOpen:Boolean};get countdown(){let e=y-Date.now(),a=e/1e3,r=new Date(e),s=Math.floor(a/(3600*24)),o=r.toISOString().substring(11,13),l=r.toISOString().substring(14,16),T=r.toISOString().substring(17,19);return{days:s,hours:o,mins:l,secs:T}}connectedCallback(){super.connectedCallback(),!this.maybeDestroy()&&(this.intervalID=setInterval(()=>{this.maybeRequestUpdate()},1e3))}maybeRequestUpdate(){this.maybeDestroy()||this.requestUpdate()}destroy(){clearInterval(this.intervalID),this.remove()}maybeDestroy(){let e=y-Date.now();return!CAN_EDIT_BRACKET||e<0?(this.destroy(),!0):!1}timeCardTemplate(e,a){return t`<wa-card class="default-bg default-border"
    ...
    >`}};customElements.define("nb-countdown",P);var Pe=[["Mich.","Michigan"],["St.","State"],["U.","University"],["UConn","Connecticut"]];function h(i){for(let[e,a]of Pe)if(i.endsWith(e))return i.replace(e,a);return i}var Ue=fetch(`/static/json/${CURRENT_YEAR}.standings.json`).then(i=>i.json()),je=fetch(`/static/json/${CURRENT_YEAR}.conference.json`).then(i=>i.json());async function U(i){let e=h(i.name),[a,r]=await Promise.all([Ue,je]),s=a[e],o=r[e];return!s||!o?(console.log("NO STATS FOR",i,e),null):{...s,...o}}var Re=fetch(`/static/json/${CURRENT_YEAR}.games.json`).then(i=>i.json());function b(i,e){let a=[0,0,0];for(let r of e)r.tie?a[2]+=1:r.winner===i?a[0]+=1:a[1]+=1;return a}async function Ne(i,e){let a=h(i.name),r=h(e.name),s=await Re,o=[];for(let l of s)(l.home===a&&l.away===r||l.home===r&&l.away===a)&&o.push(l);return[b(a,o),b(r,o)]}async function j(i){let e=h(i.name),a=await Re,r=[],s=[],o=[];for(let l of a)l.home===e?l.neutralSite?o.push(l):r.push(l):l.away===e&&(l.neutralSite?o.push(l):s.push(l));return[b(e,r),b(e,s),b(e,o)]}var A=class extends n{static properties={topTeam:{type:Object},topTeamStats:{type:Object},bottomTeam:{type:Object},bottomTeamStats:{type:Object},game:{type:String}};requestUpdate(e,a){return e==="topTeam"&&(this.topTeamStats=null,this.requestTopTeamStats()),e==="bottomTeam"&&(this.bottomTeamStats=null,this.requestBottomTeamStats()),(e==="topTeamStats"||e==="bottomTeamStats")&&this.topTeamStats&&this.bottomTeamStats&&this.requestHeadToHeadStats(),super.requestUpdate(e,a)}async requestHeadToHeadStats(){if(!(this.topTeam?.team&&this.bottomTeam?.team&&this.topTeamStats&&this.bottomTeamStats))return;let[e,a]=await Ne(this.topTeam.team,this.bottomTeam.team);this.topTeamStats.h2h=e,this.bottomTeamStats.h2h=a,this.requestUpdate()}statsTemplate(e,a=!1){return t`<small class="${a?"self-end":""}"
        >NPI: ${e.rank}</small
      >
      ...
      ${e.h2h?t`<div
          ...
          </div>`:null}`}async requestTopTeamStats(){if(!this.topTeam.team){this.topTeamStats=null;return}let e=await U(this.topTeam.team),a=await j(this.topTeam.team);this.topTeamStats={...e,records:a}}async requestBottomTeamStats(){if(!this.bottomTeam.team){this.bottomTeamStats=null;return}let e=await U(this.bottomTeam.team),a=await j(this.bottomTeam.team);this.bottomTeamStats={...e,records:a}}topTeamStatsTemplate(){return this.topTeamStats?t`<div class="grow flex flex-col min-w-33">
    ...
    </div>`}};customElements.define("nb-edit-bracket",f);var L=class i extends w{static properties={topTeamGoals:{type:Number,converter:i.goalConverter},bottomTeamGoals:{type:Number,converter:i.goalConverter}};static goalConverter(e,a){return e.length?Number(e):null}teamTemplate(e){return e?.team?e.team.name:""}topScoreInput(){return t`<input
      type="number"
    ...
    </wa-card>`}};customElements.define("nb-edit-correct-matchup",L);var q=class extends f{matchupTagName="nb-edit-correct-matchup";maybeToggleSaveButton(){}matchupTemplate(e){return t`<nb-edit-correct-matchup
      id=${e.game}
    ...
    </div>`}};customElements.define("nb-edit-correct-bracket",q);var M=class extends n{static properties={email:{type:String}};connectedCallback(){super.connectedCallback();let a=new URLSearchParams(window.location.search).get("next");a&&window.localStorage.setItem("next",a)}nativeTemplate(){return t`<wa-card>
    ...
    >
      ${e}
    </div>`}};customElements.define("nb-bracket-column",$);var Y=class extends ${groupsTemplate(){return this.bracket.group_brackets.length?t`<nb-group-bracket-details
      size="small"
    ...
    >
    ...
    </wa-dialog>`}};var F=class extends m{static properties={bracket:{type:Object}};lableTemplate(){return t`Delete bracket named "${this.bracket.name}"?`}contentTemplate(){return t`<form
      ...
      >`}};customElements.define("nb-delete-bracket",F);var H=class extends c{sortBrackets(){for(let e of this.brackets)e.rank=e.group_bracket?.group_rank;super.sortBrackets()}};customElements.define("nb-group-standings-grid",H);var z=class extends n{static properties={bracket:{type:Object}};render(){return t`<div class="wa-cluster">
      <img
        class="standings-img"
      ...
      ></nb-bracket-card-content
    ></wa-card>`}};customElements.define("nb-bracket-card",K);var V=class extends n{static properties={group:{type:Object}};render(){return t`<div class="wa-cluster">
      <wa-icon
    ...
    ></wa-card>`}};customElements.define("nb-group-card",J);var k=class extends n{static properties={label:{type:String},value:{type:String},name:{type:String},icon:{type:String},form:{type:String}};static queries={input:"input"};iconTemplate(){return this.icon?t`<wa-icon name=${this.icon}></wa-icon>`:null}labelTemplate(){return t`<span>${this.label}</span>`}render(){return t`<wa-card class="nb-radio-item">
    ...
    </wa-card>`}};customElements.define("nb-radio-item",k);var Q=class extends k{static properties={bracket:{type:Object}};connectedCallback(){super.connectedCallback(),this.value=this.bracket.id}iconTemplate(){return null}labelTemplate(){return t`<nb-bracket-card-content
      .bracket=${this.bracket}
    ></nb-bracket-card-content>`}};customElements.define("nb-bracket-radio-item",Q);var X=class extends k{static properties={group:{type:Object}};connectedCallback(){super.connectedCallback(),this.value=this.group.id}iconTemplate(){return null}labelTemplate(){return t`<nb-group-card-content
      .group=${this.group}
    ></nb-group-card-content>`}};customElements.define("nb-group-radio-item",X);var Z=class extends m{static properties={myBrackets:{type:Object},group:{type:Object}};get joinedBrackets(){return this.group.brackets.filter(e=>e.user_id===CURRENT_USER.id)}get bracketsCanJoin(){return this.myBrackets.filter(e=>!this.joinedBrackets.find(a=>a.id===e.id))}groupTemplate(){return t`<div class="wa-stack">
      <p>Joining group:</p>
    ...
    >`}};customElements.define("nb-group-add-bracket",Z);var ee=class extends m{static properties={bracket:{type:Object},group:{type:Object}};lableTemplate(){return t`Remove Bracket?`}contentTemplate(){return t`<form
    ...
    ></nb-group-radio-item>`}groupsTemplate(){let e=this.groupsCanJoin.map(a=>this.groupTemplate(a));return e.length===0&&(e=t`<div class="flex justify-center">
      ...
      >Add Bracket</wa-button
    >`}};customElements.define("nb-add-bracket-to-group",ie);var se=class extends n{static properties={groupBrackets:{type:Array},size:{type:String}};groupBracketTemplate(e){return t`<wa-card class="group-bracket-card ${this.size} default-border"
      ><a
    ...
    </wa-card>`:null}};customElements.define("nb-preferences",ue);var De="Email taken. Please choose a different email or login.",Le="Username taken. Please choose a different username.",v=class extends n{static properties={email:{type:String},emailValid:{type:Boolean},usernameValid:{type:Boolean}};static queries={emailInput:"#email",usernameInput:"#username",submitButton:"#submitButtn"};async checkEmailUnique(e){let a=await fetch(EMAIL_UNIQUE_URL+"?"+new URLSearchParams({email:e}));return a=await a.json(),a}async checkUsernameUnique(e){let a=await fetch(USERNAME_UNIQUE_URL+"?"+new URLSearchParams({username:e}));return a=await a.json(),a}async handleEmailInput(){this.emailTask||(this.emailTask=new d(async()=>{let e=this.emailInput.value,a=await this.checkEmailUnique(e);console.log("email is unique",a.isUnique),a.isUnique?(this.emailInput.hint="",this.emailValid=!0):(this.emailInput.hint=De,this.emailValid=!1)},300)),this.emailTask.arm()}async handleUsernameInput(){this.usernameTask||(this.usernameTask=new d(async()=>{let e=this.usernameInput.value,a=await this.checkUsernameUnique(e);console.log("username is unique",a.isUnique),a.isUnique?(this.usernameInput.hint="",this.usernameValid=!0):(this.usernameInput.hint=Le,this.usernameValid=!1)},300)),this.usernameTask.arm()}render(){return t`<wa-card>
    ...
    </div>`}};customElements.define("nb-bracket",we);var fe=class extends m{static properties={group:{type:Object},controller:{type:Object}};handleHide(){this.controller.show()}lableTemplate(){return t`Delete this group?`}contentTemplate(){return t`<form
    ...
    >
      ...
      <wa-button
        ...
        disabled
        >Save</wa-button
      >`}async connectedCallback(){super.connectedCallback(),await this.updateComplete,this.private=this.group.is_private}};customElements.define("nb-edit-group",ke);var ve=class extends m{static properties={group:{type:Object}};lableTemplate(){return t`Join ${this.group.name}`}contentTemplate(){return t`<form
...
//# sourceMappingURL=nb.LWUHQ34U.mjs.map
