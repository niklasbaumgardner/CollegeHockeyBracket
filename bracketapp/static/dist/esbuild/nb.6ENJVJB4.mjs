import{d as t,h as x,j as ye,k as f}from"./chunk-XGX4NMMB.mjs";import{a as _e,b as $e,c as Ee,d as w}from"./chunk-YTZDMNTQ.mjs";import{a as u,c as Ce,d as xe,e as Se,f as S,i as Re}from"./chunk-QPYYSUDA.mjs";function Ne(i,e){return()=>i.querySelector(e)}function Ie(i,e){return()=>i.querySelectorAll(e)}var n=class extends ye{constructor(){super();let{queries:e}=this.constructor;if(e)for(let[a,r]of Object.entries(e))r.all?Object.defineProperty(this,a,{get:Ie(this,r.all)}):Object.defineProperty(this,a,{get:Ne(this,r)})}createRenderRoot(){return this}};var R=class extends n{render(){return t`<wa-card
    ...
    />`}popoverTemplate(){return this.winnerTop?.team&&this.winnerBottom?.team?t`<wa-button
      ...
      <nb-matchup-info
        ...
        game=${this.game}
      ></nb-matchup-info>
    </div>`}};customElements.define("nb-edit-matchup",k);var O=class extends n{static properties={default:{type:Object},teams:{type:Array}};teamOptions(e,a){let r=this.teams.map(s=>t`<option
          value=${s.id}
    ...
    >
      ...
      .brackets=${this.brackets}
      year=${this.year}
    ></nb-group-standings-grid>`}render(){return this.year?t`${super.render()}`:null}};customElements.define("nb-group-standings",ve);var je="https://www.collegehockeynews.com/external/widgets/ajaxprocess/makeJSONP.php?datafile=liveScoreboardData.json&callback=",Te=class extends n{static properties={games:{type:Array}};static queries={drawer:"wa-drawer",desktopEl:".wa-desktop-only",mobileEl:".wa-mobile-only",closeButton:"#close-scoreboard"};connectedCallback(){super.connectedCallback(),this.init()}get isOpen(){return this.checkVisibility()}get isMobile(){return this.mobileEl.checkVisibility()}get isDesktop(){return this.desktopEl.checkVisibility()}async getJsonP(){let e="callback_"+Math.floor(Date.now()/6e4),a=localStorage.getItem("previousCallBack");if(a===e)this.games=JSON.parse(localStorage.getItem(a));else{let r=document.createElement("script");r.src=je+e,document.head.append(r),this.games=await new Promise(s=>{window[e]=o=>{localStorage.removeItem(a),localStorage.setItem("previousCallBack",e),localStorage.setItem(e,JSON.stringify(o)),s(o),r.remove()}})}}async getJson(){let r=await(await fetch("./static/json/dev/live.final.json")).json();this.games=r}async init(){let e=(localStorage.getItem("scoreboardState")??"open")==="open";if(this.button=document.getElementById("scoreboard-button"),this.button.hidden=!0,await this.getJsonP(),this.games.length===0){this.button.remove(),this.remove();return}await this.updateComplete,this.button.addEventListener("click",this),this.closeButton.addEventListener("click",this),this.isMobile&&(this.button.hidden=!1),this.isDesktop&&!e&&this.hide()}handleEvent(e){e.type==="click"&&(e.target===this.button?this.isMobile?this.drawer.open=!this.drawer.open:this.show():e.target===this.closeButton&&this.hide())}show(){this.desktopEl.hidden=!1,this.button.hidden=!0,localStorage.setItem("scoreboardState","open")}hide(){this.desktopEl.hidden=!0,this.button.hidden=!1,localStorage.setItem("scoreboardState","closed")}teamTemplate(e,a){return t`<div class="flex justify-between wa-heading-m">
      <div class="flex">
        <div class="wa-body-s min-w-[4ch]">${a.npi_rank?`(${a.npi_rank})`:""}</div>
        ${e}
      </div>
      <div">${a.score}</div>
    </div>`}gameTemplate(e){let a="warning";return e.gamestatus==="Final"?a="neutral":e.gamestatus.includes("Per.")&&(a="danger"),t`<div>
...
//# sourceMappingURL=nb.6ENJVJB4.mjs.map
