import{d as t,h as S,j as Se,k as f}from"./chunk-XGX4NMMB.mjs";import{a as Re,b as Be,c as Ne,d as b}from"./chunk-YTZDMNTQ.mjs";import{a as h,c as Ie,d as Pe,e as Ae,f as R,i as Ge}from"./chunk-P43BX2YJ.mjs";function Oe(i,e){return()=>i.querySelector(e)}function je(i,e){return()=>i.querySelectorAll(e)}var n=class extends Se{constructor(){super();let{queries:e}=this.constructor;if(e)for(let[a,r]of Object.entries(e))r.all?Object.defineProperty(this,a,{get:je(this,r.all)}):Object.defineProperty(this,a,{get:Oe(this,r)})}createRenderRoot(){return this}};var B=class extends n{render(){return t`<wa-card
      ><form method="POST">
        <div class="wa-stack">
          <wa-input
            name="team"
    ...
    </wa-card>`}};customElements.define("nb-archive-years",I);var P=class extends n{static properties={message:{type:String},category:{type:String}};connectedCallback(){super.connectedCallback(),this.removeSelfTask=new h(()=>this.removeSelf(),5e3,{finalizeBeforeUnload:!0}),this.removeSelfTask.arm()}removeSelf(){this.remove()}render(){return t`<wa-callout variant=${this.category}>
      ${this.message}
      <wa-button
        class="icon-button"
        appearance="plain"
        ...
        ></wa-icon
      ></wa-button>
    </wa-callout>`}};customElements.define("nb-alert",P);var A=class extends n{static properties={messages:{type:Array,converter:(e,a)=>!e||!e.length||e==="[]"?[]:[...e.match(/(?<=\()[^()]+(?=\))/g)].map(r=>r.replaceAll("'","").split(", "))}};render(){return this.messages.length?t`<div id="alerts" class="wa-stack items-center">
    ...
    >
    ...
    </wa-dialog>`}};var U=class extends c{lableTemplate(){return t`Subscribe to email updates?`}contentTemplate(){return t`<div>Updates will be sent to ${CURRENT_USER.email}</div>
      <div>
        Want to use a different email? Update your email
        <a href=${PROFILE_URL}>here</a>
      </div>`}footerTemplate(){return t`${this.cancelButtonTemplate()}<wa-button
        class="grow"
        variant="brand"
        >Subscribe</wa-button
      >`}};customElements.define("nb-email-updates",U);var $=new Date("2026-03-22T20:00:00.000Z"),k=new Date("2026-03-26T18:00:00.000Z"),O=class extends n{static properties={type:{type:String,reflect:!0},size:{type:String}};extractFromMs(e){let a=e/1e3,r=new Date(e),s=Math.floor(a/(3600*24)),o=r.toISOString().substring(11,13),m=r.toISOString().substring(14,16),l=r.toISOString().substring(17,19);return{days:s,hours:o,mins:m,secs:l}}get timeToClose(){let e=k-Date.now();return this.extractFromMs(e)}get timeToOpen(){let e=$-Date.now();return this.extractFromMs(e)}connectedCallback(){super.connectedCallback();let e=k-Date.now();!CAN_EDIT_BRACKET&&e<0&&this.remove()}startInterval(){this.intervalID||(this.intervalID=setInterval(()=>{this.intervalHandler()},1e3))}intervalHandler(){let e=$-Date.now();!CAN_EDIT_BRACKET&&e<0&&(setTimeout(()=>location.reload(),1e4),this.stopInterval()),this.requestUpdate()}stopInterval(){clearInterval(this.intervalID)}messageTemplate(){let e=$-Date.now(),a=k-Date.now(),r=36e5;if(!CAN_EDIT_BRACKET&&e>0)return this.type="timeToOpen",this.startInterval(),t`${CURRENT_YEAR} brackets will be available on
        <wa-format-date
          date=${$}
          month="long"
          day="numeric"
      ...
      ><div class="wa-stack items-center">
        ...
        <span class="uppercase">${a+(e==1?"":"s")}</span>
      </div></wa-card
    >`}timerTemplate(){let{days:e,hours:a,mins:r,secs:s}=this.type==="timeToOpen"?this.timeToOpen:this.timeToClose;return e<0?null:t`<div
      class="wa-cluster ${this.size==="small"?"gap-(--wa-space-xs)":""}"
    >
      ${[[e,"day"],[a,"hour"],[r,"minute"],[s,"second"]].map(o=>this.timeCardTemplate(...o))}
    </div>`}handleSubcribeClick(){if(!this.emailUpdatesEl){let e=document.createElement("nb-email-updates");document.body.append(e),this.emailUpdatesEl=e}this.emailUpdatesEl.show()}subscribeButtonTemplate(){}render(){return this.size==="small"?t`<div class="wa-cluster">
        <div>${this.messageTemplate()}</div>
        ${this.timerTemplate()}
      </div>`:t`<wa-card class="default-bg default-border"
      ><div class="wa-stack items-center">
        <h4>${this.messageTemplate()}</h4>
        ${this.timerTemplate()}
      </div></wa-card
    >`}};customElements.define("nb-countdown",O);var Le=[["Mich.","Michigan"],["St.","State"],["U.","University"],["UConn","Connecticut"],["Minn. Duluth","Minnesota-Duluth"]];function g(i){for(let[e,a]of Le)if(i.endsWith(e))return i.replace(e,a);return i}var j=`${CURRENT_YEAR}.games`,De=`/static/json/${CURRENT_YEAR}.games.json`,L=`${CURRENT_YEAR}.npi`,Me=`/static/json/${CURRENT_YEAR}.npi.json`,D=`${CURRENT_YEAR}.krach`,qe=`/static/json/${CURRENT_YEAR}.krach.json`,M=`${CURRENT_YEAR}.conference`,Fe=`/static/json/${CURRENT_YEAR}.conference.json`,q=class{constructor(){this.cache=new Map,this.gamesRequestPromise=null,this.gamesJsonPromise=null,this.npiRequestPromise=null,this.npiJsonPromise=null,this.krachRequestPromise=null,this.krachJsonPromise=null,this.confStandingsRequestPromise=null,this.confStandingsJsonPromise=null}async getGames(){if(this.cache.has(j)&&this.cache.get(j),!this.gamesRequestPromise){let r=STATIC_FILE_MAP[De];this.gamesRequestPromise=fetch(r)}let e=await this.gamesRequestPromise;this.gamesJsonPromise||(this.gamesJsonPromise=e.json());let a=await this.gamesJsonPromise;return this.cache.set(j,a),a}async getKrach(){if(this.cache.has(D)&&this.cache.get(D),!this.krachRequestPromise){let r=STATIC_FILE_MAP[qe];this.krachRequestPromise=fetch(r)}let e=await this.krachRequestPromise;this.krachJsonPromise||(this.krachJsonPromise=e.json());let a=await this.krachJsonPromise;return this.cache.set(D,a),a}async getNPI(){if(this.cache.has(L)&&this.cache.get(L),!this.npiRequestPromise){let r=STATIC_FILE_MAP[Me];this.npiRequestPromise=fetch(r)}let e=await this.npiRequestPromise;this.npiJsonPromise||(this.npiJsonPromise=e.json());let a=await this.npiJsonPromise;return this.cache.set(L,a),a}async getConference(){if(this.cache.has(M)&&this.cache.get(M),!this.confStandingsRequestPromise){let r=STATIC_FILE_MAP[Fe];this.confStandingsRequestPromise=fetch(r)}let e=await this.confStandingsRequestPromise;this.confStandingsJsonPromise||(this.confStandingsJsonPromise=e.json());let a=await this.confStandingsJsonPromise;return this.cache.set(M,a),a}getRecordFromGames(e,a){let r=[0,0,0];for(let s of a)s.tie?r[2]+=1:s.winner===e?r[0]+=1:r[1]+=1;return r}async getHeadToHeadGames(e,a){let r=g(e.name),s=g(a.name),o=await this.getGames(),m=[];for(let l of o)(l.home===r&&l.away===s||l.home===s&&l.away===r)&&m.push(l);return[this.getRecordFromGames(r,m),this.getRecordFromGames(s,m)]}async getHomeAwayNeutralRecord(e){let a=g(e.name),r=await this.getGames(),s=[],o=[],m=[];for(let l of r)l.home===a?l.neutralSite?m.push(l):s.push(l):l.away===a&&(l.neutralSite?m.push(l):o.push(l));return[this.getRecordFromGames(a,s),this.getRecordFromGames(a,o),this.getRecordFromGames(a,m)]}async getStandingsForTeam(e){let a=g(e.name),[r,s,o]=await Promise.all([this.getKrach(),this.getConference(),this.getNPI()]),m=r[a],l=s[a],p=o[a];return!m||!l||!p?(console.log("NO STATS FOR",e,a),null):{...m,...l,...p}}async getNotableWins(e){let a=g(e.name),r=await this.getGames(),s=[];for(let o of r)(o.home===a||o.away===a)&&o.winner===a&&o.notable&&s.push(o);return s}async getNPIRank(e){let a=g(e.name);return(await this.getNPI())[a].npi.rank}},d=new q;var F=class extends n{static properties={topTeam:{type:Object},topTeamStats:{type:Object},bottomTeam:{type:Object},bottomTeamStats:{type:Object},game:{type:String}};requestUpdate(e,a){return e==="topTeam"&&(this.topTeamStats=null,this.requestTopTeamStats()),e==="bottomTeam"&&(this.bottomTeamStats=null,this.requestBottomTeamStats()),(e==="topTeamStats"||e==="bottomTeamStats")&&this.topTeamStats&&this.bottomTeamStats&&this.requestHeadToHeadStats(),super.requestUpdate(e,a)}async requestHeadToHeadStats(){if(!(this.topTeam?.team&&this.bottomTeam?.team&&this.topTeamStats&&this.bottomTeamStats))return;let[e,a]=await d.getHeadToHeadGames(this.topTeam.team,this.bottomTeam.team);this.topTeamStats.h2h=e,this.bottomTeamStats.h2h=a,this.requestUpdate()}statsTemplate(e,a=!1){let r=a?"self-end text-right":"";return t`<small class="${r}">NPI: ${e.npi.rank}</small>
      <small class="${r}"
    ...
    </wa-card>`}};customElements.define("nb-edit-correct-matchup",W);var H=class extends T{matchupTagName="nb-edit-correct-matchup";maybeToggleSaveButton(){}matchupTemplate(e){return t`<nb-edit-correct-matchup
      id=${e.game}
    ...
    />`}groupsTemplate(){return null}clickableTemplate(){return t`<a
      ...
      ><span class="standings-username">${this.bracket.user.username}</span>`}safeTemplate(){return t`<div class="flex gap-(--wa-space-xs)">
    ...
    >
      ${e}
    </div>`}};customElements.define("nb-bracket-column",C);var J=class extends C{groupsTemplate(){return this.bracket.group_brackets.length?t`<nb-group-bracket-details
      size="small"
    ...
    ></nb-standings-grid>`}render(){return this.year?t`<wa-card>
      ...
      >`}};customElements.define("nb-delete-bracket",V);var Q=class extends u{sortBrackets(){for(let e of this.brackets)e.rank=e.group_bracket?.group_rank;super.sortBrackets()}};customElements.define("nb-group-standings-grid",Q);var X=class extends n{static properties={bracket:{type:Object}};render(){return t`<div class="wa-cluster">
      <img
        class="standings-img"
    ...
    </div>`}};customElements.define("nb-bracket-card-content",X);var Z=class extends n{static properties={bracket:{type:Object}};render(){return t`<wa-card style="--spacing:var(--wa-space-s);"
      ><nb-bracket-card-content
        .bracket=${this.bracket}
      ></nb-bracket-card-content
    ></wa-card>`}};customElements.define("nb-bracket-card",Z);var ee=class extends n{static properties={group:{type:Object}};render(){return t`<div class="wa-cluster">
      <wa-icon
    ...
    </div>`}};customElements.define("nb-group-card-content",ee);var te=class extends n{static properties={group:{type:Object}};render(){return t`<wa-card style="--spacing:var(--wa-space-s);"
    ...
    </wa-card>`}};customElements.define("nb-radio-item",_);var ae=class extends _{static properties={bracket:{type:Object}};connectedCallback(){super.connectedCallback(),this.value=this.bracket.id}iconTemplate(){return null}labelTemplate(){return t`<nb-bracket-card-content
      .bracket=${this.bracket}
    ></nb-bracket-card-content>`}};customElements.define("nb-bracket-radio-item",ae);var re=class extends _{static properties={group:{type:Object}};connectedCallback(){super.connectedCallback(),this.value=this.group.id}iconTemplate(){return null}labelTemplate(){return t`<nb-group-card-content
      .group=${this.group}
    ></nb-group-card-content>`}};customElements.define("nb-group-radio-item",re);var se=class extends c{static properties={myBrackets:{type:Object},group:{type:Object}};get joinedBrackets(){return this.group.brackets.filter(e=>e.user_id===CURRENT_USER.id)}get bracketsCanJoin(){return this.myBrackets.filter(e=>!this.joinedBrackets.find(a=>a.id===e.id))}groupTemplate(){return t`<div class="wa-stack">
      <p>Joining group:</p>
    ...
    >`}};customElements.define("nb-group-add-bracket",se);var ie=class extends c{static properties={bracket:{type:Object},group:{type:Object}};lableTemplate(){return t`Remove Bracket?`}contentTemplate(){return t`<form
    ...
    ></nb-group-radio-item>`}groupsTemplate(){let e=this.groupsCanJoin.map(a=>this.groupTemplate(a));return e.length===0&&(e=t`<div class="flex justify-center">
      ...
      >Add Bracket</wa-button
    >`}};customElements.define("nb-add-bracket-to-group",me);var ce=class extends n{static properties={groupBrackets:{type:Array},size:{type:String}};groupBracketTemplate(e){return t`<wa-card class="group-bracket-card ${this.size} default-border"
      ><a
    ...
    ></wa-callout>`}messageTemplate(){let e=null;return this.canEditThisYearsBrackets&&(e=t`<nb-countdown></nb-countdown>`),this.year!==CURRENT_YEAR&&(e=t`${e}${this.previewYearMessage()}`),e}newBracketButtonTemplate(){return this.canCreateBracket?t`<wa-button
        variant="brand"
        appearance="outlined"
    ...
    >`}};customElements.define("nb-create-group",ge);var we=class extends n{static properties={email:{type:String}};static queries={};render(){return t`<wa-card>
      ...
      <wa-button
        ...
        >Reset</wa-button
      >
    </div>`}bgNumberRadioTemplate(){if(!this.backgroundColorSelect){this.updateComplete.then(()=>this.requestUpdate());return}let e=this.backgroundColorSelect.value?.toLowerCase();if(R.includes(e))return Ae.map(a=>t`<input
    ...
    </div>`}variantsTemplate(){return Pe.map(e=>t`<wa-select
          with-clear
          id="${e}-color"
          name=${e}
          label="${x(e)} Color"
          @input=${this.handleVariantChange}
          >${Ie.map(a=>t`<wa-option
    ...
    </wa-card>`:null}};customElements.define("nb-preferences",fe);var He="Email taken. Please choose a different email or login.",Ke="Username taken. Please choose a different username.",y=class extends n{static properties={email:{type:String},emailValid:{type:Boolean},usernameValid:{type:Boolean}};static queries={emailInput:"#email",usernameInput:"#username",submitButton:"#submitButtn"};async checkEmailUnique(e){let a=await fetch(EMAIL_UNIQUE_URL+"?"+new URLSearchParams({email:e}));return a=await a.json(),a}async checkUsernameUnique(e){let a=await fetch(USERNAME_UNIQUE_URL+"?"+new URLSearchParams({username:e}));return a=await a.json(),a}async handleEmailInput(){this.emailTask||(this.emailTask=new h(async()=>{let e=this.emailInput.value,a=await this.checkEmailUnique(e);console.log("email is unique",a.isUnique),a.isUnique?(this.emailInput.hint="",this.emailValid=!0):(this.emailInput.hint=He,this.emailValid=!1)},300)),this.emailTask.arm()}async handleUsernameInput(){this.usernameTask||(this.usernameTask=new h(async()=>{let e=this.usernameInput.value,a=await this.checkUsernameUnique(e);console.log("username is unique",a.isUnique),a.isUnique?(this.usernameInput.hint="",this.usernameValid=!0):(this.usernameInput.hint=Ke,this.usernameValid=!1)},300)),this.usernameTask.arm()}render(){return t`<wa-card>
    ...
    </wa-card>`}};customElements.define("nb-signup",y);import ze from"https://cdn.jsdelivr.net/npm/profanity-cleaner@0.0.3/+esm";var ke=class extends y{static properties={username:{type:String}};async handleEmailInput(){if(this.emailInput.value===this.email){this.emailInput.helpText="",this.emailValid=!1;return}super.handleEmailInput()}async handleUsernameInput(){if(this.usernameInput.value=ze.clean(this.usernameInput.value,{keepFirstAndLastChar:!0,replacePartialWords:!0}),await this.usernameInput.updateComplete,this.usernameInput.value===this.username){this.usernameInput.helpText="",this.usernameValid=!1;return}super.handleUsernameInput()}render(){return t`<wa-card>
    ...
    ></nb-matchup>`}roundOneLeftTemplate(){return t`<div class="round-one">
      ...
      .groupBrackets=${this.bracket.group_brackets}
    ></nb-group-bracket-details>`:null}calloutTemplate(){return this.bracket&&this.bracket.year!==CURRENT_YEAR?t`<wa-callout
        >You are viewing a bracket from ${this.bracket.year}</wa-callout
      >`:null}topCardStats(){if(this.type==="correct"){let e;return this.correct.winner_team.id===this.correct.games.game13.winner_team.id?e=this.correct.games.game14.winner_team:e=this.correct.games.game13.winner_team,t`<div class="wa-stack items-center">
    ...
    </div>`}};customElements.define("nb-bracket",_e);var ye=class extends c{static properties={group:{type:Object},controller:{type:Object}};handleHide(){this.controller.show()}lableTemplate(){return t`Delete this group?`}contentTemplate(){return t`<form
    ...
    >
      ...
      <wa-button
        ...
        disabled
        >Save</wa-button
      >`}async connectedCallback(){super.connectedCallback(),await this.updateComplete,this.private=this.group.is_private}};customElements.define("nb-edit-group",$e);var Ee=class extends c{static properties={group:{type:Object}};lableTemplate(){return t`Join ${this.group.name}`}contentTemplate(){return t`<form
    ...
    >
      ...
      .brackets=${this.brackets}
      year=${this.year}
    ></nb-group-standings-grid>`}render(){return this.year?t`${super.render()}`:null}};customElements.define("nb-group-standings",Ce);var Ve="https://www.collegehockeynews.com/external/widgets/ajaxprocess/makeJSONP.php?datafile=liveScoreboardData.json&callback=",Qe=new Set(["login","signup","profile","preferences","password_request","password_reset"]),Xe=new Set(["leaderboard","group","bracket"]),xe=class extends n{static properties={games:{type:Array}};static queries={drawer:"wa-drawer",desktopEl:"#desktop",mobileEl:"#mobile",closeButton:"#close-scoreboard"};connectedCallback(){super.connectedCallback(),this.init()}get isOpen(){return this.checkVisibility()}get isMobile(){return this.mobileEl.checkVisibility()}get isDesktop(){return this.desktopEl.checkVisibility()}async getJsonP(){let e="callback_"+Math.floor(Date.now()/6e4),a=localStorage.getItem("previousCallBack");if(a===e)this.games=JSON.parse(localStorage.getItem(a));else{let r=document.createElement("script");r.src=Ve+e,document.head.append(r),this.games=await new Promise(s=>{window[e]=o=>{localStorage.removeItem(a),localStorage.setItem("previousCallBack",e),localStorage.setItem(e,JSON.stringify(o)),s(o),r.remove()}})}}async getJson(){let r=await(await fetch("/static/json/dev/live.3-5.json")).json();this.games=r}sortGames(){this.games.sort((e,a)=>{let r=0;if(e.gamestatus.includes("Final")&&a.gamestatus.includes("Final"))r=0;else if(e.gamestatus===a.gamestatus)r=0;else if(e.gamestatus.includes("Final"))r=1;else if(a.gamestatus.includes("Final"))r=-1;else{let s=!isNaN(e.gamestatus[0]),o=!isNaN(a.gamestatus[0]);s&&!o?r=1:!s&&o?r=-1:r=e.gamestatus.localeCompare(a.gamestatus)}return r})}shouldShowOnPage(){let a=new URL(location.href).pathname.split("/").at(1);return!CURRENT_USER?.id||Qe.has(a)?-1:Xe.has(a)?1:0}async init(){this.button=document.getElementById("scoreboard-button");let e=this.shouldShowOnPage();if(e===-1){this.button.remove(),this.remove();return}let a=(localStorage.getItem("scoreboardState")??"open")==="open"&&e===1;if(await this.getJsonP(),this.games.length===0){this.button.remove(),this.remove();return}else this.style.removeProperty("width");this.sortGames(),this.requestUpdate(),this.button.addEventListener("click",this),this.closeButton.addEventListener("click",this),this.isMobile&&(this.button.hidden=!1),this.isDesktop&&!a&&this.hide()}handleEvent(e){e.type==="click"&&(e.target===this.button?this.isMobile?this.drawer.open=!this.drawer.open:this.show():e.target===this.closeButton&&this.hide())}show(){this.desktopEl.hidden=!1,this.button.hidden=!0,localStorage.setItem("scoreboardState","open")}hide(){this.desktopEl.hidden=!0,this.button.hidden=!1,localStorage.setItem("scoreboardState","closed")}teamTemplate(e,a){return t`<div class="flex justify-between wa-heading-m">
      <div class="flex">
    ...
    </aside>`}};customElements.define("chn-scoreboard",xe);
//# sourceMappingURL=nb.YVE6M643.mjs.map
