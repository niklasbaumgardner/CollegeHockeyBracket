import{d as t,h as C,j as _e,k as x}from"./chunk-XGX4NMMB.mjs";import{a as $e,b as Ee,c as Ce,d as u}from"./chunk-YTZDMNTQ.mjs";import{a as d,b as xe,d as Se,e as Be,f as Re,g as S,j as Ne}from"./chunk-47VUWAMQ.mjs";function Ue(s,e){return()=>s.querySelector(e)}function je(s,e){return()=>s.querySelectorAll(e)}var n=class extends _e{constructor(){super();let{queries:e}=this.constructor;if(e)for(let[a,r]of Object.entries(e))r.all?Object.defineProperty(this,a,{get:je(this,r.all)}):Object.defineProperty(this,a,{get:Ue(this,r)})}createRenderRoot(){return this}};var B=class extends n{render(){return t`<wa-card
    ...
    >`}};customElements.define("nb-add-team",B);var R=class extends n{render(){return t`<wa-card>
      <div class="wa-stack">
    ...
    </wa-card>`}};customElements.define("nb-admin",R);var N=class extends n{static properties={archivedYears:{type:Array}};imgTemplate(e){return e?t`<img
      class="w-[75px] h-[75px]"
    ...
    >`}};customElements.define("nb-back-button",G);var I=class extends n{static properties={message:{type:String},category:{type:String}};connectedCallback(){super.connectedCallback(),this.removeSelfTask=new d(()=>this.removeSelf(),5e3,{finalizeBeforeUnload:!0}),this.removeSelfTask.arm()}removeSelf(){this.remove()}render(){return t`<wa-callout variant=${this.category}>
      ...
      >
    </wa-dropdown>`}};customElements.define("nb-theme-selector",U);var _=new Date("2025-03-27T18:00:00.000Z"),j=class extends n{static properties={bracketsOpen:Boolean};get countdown(){let e=_-Date.now(),a=e/1e3,r=new Date(e),i=Math.floor(a/(3600*24)),o=r.toISOString().substring(11,13),l=r.toISOString().substring(14,16),y=r.toISOString().substring(17,19);return{days:i,hours:o,mins:l,secs:y}}connectedCallback(){super.connectedCallback(),!this.maybeDestroy()&&(this.intervalID=setInterval(()=>{this.maybeRequestUpdate()},1e3))}maybeRequestUpdate(){this.maybeDestroy()||this.requestUpdate()}destroy(){clearInterval(this.intervalID),this.remove()}maybeDestroy(){let e=_-Date.now();return!CAN_EDIT_BRACKET||e<0?(this.destroy(),!0):!1}timeCardTemplate(e,a){return t`<wa-card class="default-bg default-border"
    ...
    >`}};customElements.define("nb-countdown",j);var Pe=[["Mich.","Michigan"],["St.","State"],["U.","University"],["UConn","Connecticut"]];function h(s){for(let[e,a]of Pe)if(s.endsWith(e))return s.replace(e,a);return s}var Ae=fetch(`/static/json/${CURRENT_YEAR}.standings.json`).then(s=>s.json()),De=fetch(`/static/json/${CURRENT_YEAR}.conference.json`).then(s=>s.json());async function P(s){let e=h(s.name),[a,r]=await Promise.all([Ae,De]),i=a[e],o=r[e];return!i||!o?(console.log("NO STATS FOR",s,e),null):{...i,...o}}var Ge=fetch(`/static/json/${CURRENT_YEAR}.games.json`).then(s=>s.json());function b(s,e){let a=[0,0,0];for(let r of e)r.tie?a[2]+=1:r.winner===s?a[0]+=1:a[1]+=1;return a}async function Ie(s,e){let a=h(s.name),r=h(e.name),i=await Ge,o=[];for(let l of i)(l.home===a&&l.away===r||l.home===r&&l.away===a)&&o.push(l);return[b(a,o),b(r,o)]}async function A(s){let e=h(s.name),a=await Ge,r=[],i=[],o=[];for(let l of a)l.home===e?l.neutralSite?o.push(l):r.push(l):l.away===e&&(l.neutralSite?o.push(l):i.push(l));return[b(e,r),b(e,i),b(e,o)]}var D=class extends n{static properties={topTeam:{type:Object},topTeamStats:{type:Object},bottomTeam:{type:Object},bottomTeamStats:{type:Object},game:{type:String}};requestUpdate(e,a){return e==="topTeam"&&(this.topTeamStats=null,this.requestTopTeamStats()),e==="bottomTeam"&&(this.bottomTeamStats=null,this.requestBottomTeamStats()),(e==="topTeamStats"||e==="bottomTeamStats")&&this.topTeamStats&&this.bottomTeamStats&&this.requestHeadToHeadStats(),super.requestUpdate(e,a)}async requestHeadToHeadStats(){if(!(this.topTeam?.team&&this.bottomTeam?.team&&this.topTeamStats&&this.bottomTeamStats))return;let[e,a]=await Ie(this.topTeam.team,this.bottomTeam.team);this.topTeamStats.h2h=e,this.bottomTeamStats.h2h=a,this.requestUpdate()}statsTemplate(e,a=!1){return t`<small class="${a?"self-end":""}"
        >NPI: ${e.rank}</small
      >
    ...
    />`}popoverTemplate(){return this.winnerTop?.team&&this.winnerBottom?.team?t`<wa-button
      ...
      <nb-matchup-info
        ...
        game=${this.game}
      ></nb-matchup-info>
    </div>`}};customElements.define("nb-edit-matchup",w);var L=class extends n{static properties={default:{type:Object},teams:{type:Array}};teamOptions(e,a){let r=this.teams.map(i=>t`<option
          value=${i.id}
    ...
    </wa-card>`}};customElements.define("nb-edit-correct-matchup",q);var M=class extends f{matchupTagName="nb-edit-correct-matchup";maybeToggleSaveButton(){}matchupTemplate(e){return t`<nb-edit-correct-matchup
      id=${e.game}
    ...
    </div>`}};customElements.define("nb-edit-correct-bracket",M);var Y=class extends n{static properties={email:{type:String}};connectedCallback(){super.connectedCallback();let a=new URLSearchParams(location.search).get("next");if(a)localStorage.setItem("next",a);else{let r=localStorage.getItem("next");if(r&&!location.search.length){let i=new URL(location.href);i.searchParams.set("next",r),history.replaceState(null,"",i)}}}nativeTemplate(){return t`<wa-card>
    ...
    </wa-dialog>`}};var H=class extends m{static properties={bracket:{type:Object}};lableTemplate(){return t`Delete bracket named "${this.bracket.name}"?`}contentTemplate(){return t`<form
    ...
    </div>`}};customElements.define("nb-group-card-content",J);var Q=class extends n{static properties={group:{type:Object}};render(){return t`<wa-card style="--spacing:var(--wa-space-s);"
      ><nb-group-card-content .group=${this.group}></nb-group-card-content
    ></wa-card>`}};customElements.define("nb-group-card",Q);var k=class extends n{static properties={label:{type:String},value:{type:String},name:{type:String},icon:{type:String},form:{type:String}};static queries={input:"input"};iconTemplate(){return this.icon?t`<wa-icon name=${this.icon}></wa-icon>`:null}labelTemplate(){return t`<span>${this.label}</span>`}render(){return t`<wa-card class="nb-radio-item">
    ...
    </wa-card>`}};customElements.define("nb-radio-item",k);var X=class extends k{static properties={bracket:{type:Object}};connectedCallback(){super.connectedCallback(),this.value=this.bracket.id}iconTemplate(){return null}labelTemplate(){return t`<nb-bracket-card-content
      .bracket=${this.bracket}
    ></nb-bracket-card-content>`}};customElements.define("nb-bracket-radio-item",X);var Z=class extends k{static properties={group:{type:Object}};connectedCallback(){super.connectedCallback(),this.value=this.group.id}iconTemplate(){return null}labelTemplate(){return t`<nb-group-card-content
      .group=${this.group}
    ></nb-group-card-content>`}};customElements.define("nb-group-radio-item",Z);var ee=class extends m{static properties={myBrackets:{type:Object},group:{type:Object}};get joinedBrackets(){return this.group.brackets.filter(e=>e.user_id===CURRENT_USER.id)}get bracketsCanJoin(){return this.myBrackets.filter(e=>!this.joinedBrackets.find(a=>a.id===e.id))}groupTemplate(){return t`<div class="wa-stack">
      <p>Joining group:</p>
    ...
    >`}};customElements.define("nb-group-add-bracket",ee);var te=class extends m{static properties={bracket:{type:Object},group:{type:Object}};lableTemplate(){return t`Remove Bracket?`}contentTemplate(){return t`<form
    ...
    </div>`}};customElements.define("nb-bracket",fe);var ke=class extends m{static properties={group:{type:Object},controller:{type:Object}};handleHide(){this.controller.show()}lableTemplate(){return t`Delete this group?`}contentTemplate(){return t`<form
    ...
    >
      ...
      <wa-button
        ...
        disabled
        >Save</wa-button
      >`}async connectedCallback(){super.connectedCallback(),await this.updateComplete,this.private=this.group.is_private}};customElements.define("nb-edit-group",ve);var Te=class extends m{static properties={group:{type:Object}};lableTemplate(){return t`Join ${this.group.name}`}contentTemplate(){return t`<form
...
//# sourceMappingURL=nb.PUJFYCGA.mjs.map
